...
fn main() -> Result<()> {
    ...
    let sexpr_molecule = OrganicMolecule::from_sexpr(
        r#"
    ...
    let phenylalanine = OrganicMolecule::from_iupac("2-amino-3-phenylpropanoic acid")
        .expect("Failed to parse IUPAC name");
    println!("Phenylalanine: {:?}", phenylalanine);
    println!(
        "Phenylalanine SMILES: {}",
        phenylalanine
            .to_smiles()
            .expect("Failed to convert to SMILES")
    );

    let cyclohexylbenzene =
        OrganicMolecule::from_iupac("cyclohexylbenzene").expect("Failed to parse IUPAC name");
    println!("Cyclohexylbenzene: {:?}", cyclohexylbenzene);

    let methylbenzene =
        OrganicMolecule::from_iupac("methylbenzene").expect("Failed to parse IUPAC name");
    ...
    let strychnine = OrganicMolecule::from_smiles(
    ...
    let iupac_name = phenylalanine
        .to_iupac()
        .expect("Failed to convert to IUPAC");
    Ok(())
}
